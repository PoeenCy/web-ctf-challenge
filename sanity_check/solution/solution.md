...
Trong đoạn code được gửi kèm chall, sau khi đọc qua thì ta có thể nhận ra đây là 1 chall về type juggeling. Tức là, ta cần bypass hàm validate và ghi được poeency vào để ra flag
...
def get_flag():
    ...
    response = FLAG if "poeency" in data else "No :v"
...
Thông qua ví dụ trên, ta dễ dàng nhận ra rằng, nếu viết `poeency` ở value của dict, thì hàm for sẽ không thể check được. Tiếp theo, ta giải quyết vấn đề độ dài bằng việc set key dưới dạng `"0"*(số thứ tự của nó)`, vì int(00000...) thì bằng 0, nhưng về mặt key, thì 0000 lại khác 000.
...
## 4. Flag
`poeency{a_concident_happen_[hash]}`
